# Project Group 14 - RL & Robotics

This repository contains the code, data, and rendered results for Tasks 1, 2, and 3. Due to file size constraints, please follow the instructions below to properly run and view the project.

## Main Notebook
The primary workflow is contained in:  
**`final_notebook_group_14.ipynb`**

> **Note:** This notebook is too large to render directly on GitHub/GitLab. To view or run the content, please **download the file** and open it in a local Jupyter environment.

---

## Setup & External Data
To run the full notebook, you must download the execution logs locally:
* **Download Logs:** [Google Drive Link](https://drive.google.com/file/d/1pmn4xRDy2Ss3cryG5Cy5ldd9Yg0ePt5u/view?usp=sharing)
* *If access is restricted, please request permission via the link.*

### Required Scripts
The following scripts are essential for the notebook to function correctly:
* `record_video.py`: Handles video rendering.
* `modified_env.py`: Environment configurations.

---

## Hyperparameters
To ensure the correct parameters are applied to each task, follow these guidelines:

* **Task 1:** Found in the `hyperparams.txt` file.
* **Tasks 2 & 3:** Contained in the `.yaml` files.

---

## Repository Structure

| Folder | Content |
| :--- | :--- |
| `all_videos/` | Rendered videos for Tasks 1, 2, and 3. |
| `task1.2_graphs/` | Data files used for Task 1.2 graph generation. |
| `images/` | Alternative graphs used in the final research paper. |

---

## Final Report for Task 3

`ROB_RL_PSAI_PAPER_redacted.pdf` Contains the final report with student numbers redacted (original paper is submitted on Canvas).
